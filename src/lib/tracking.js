...
// Funnel tracking callback (set externally to avoid circular dependency)
let onEventCallback = null;

/**
 * Set callback to be called after each event
 * @param {Function} callback - Callback function(eventName, params)
 */
export function setOnEventCallback(callback) {
...
export function pushEvent(eventName, params = {}, options = {}) {
  ...
  // Call funnel tracking callback if set
  if (onEventCallback) {
    try {
      onEventCallback(eventName, params);
    } catch (e) {
      logWarning('Error in event callback:', e);
    }
  }

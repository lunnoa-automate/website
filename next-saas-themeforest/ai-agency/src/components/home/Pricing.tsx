...
import gradient51 from '@public/images/ns-img-539.png';
          ...
          <RevealAnimation delay={0.5}>
            <div>
              <label className="relative z-[10] inline-flex cursor-pointer items-center">
                ...
                />
                <span className="after:bg-accent bg-secondary dark:bg-background-9 before:border-accent/10 dark:before:border-stroke-7 relative h-[28px] w-13 rounded-[34px] before:absolute before:top-1/2 before:left-1/2 before:h-[calc(100%-10px)] before:w-[calc(100%-10px)] before:-translate-x-1/2 before:-translate-y-1/2 before:rounded-[20px] before:border before:content-[''] after:absolute after:start-[5px] after:top-1/2 after:h-6 after:w-6 after:-translate-y-1/2 after:rounded-full after:transition-all after:content-[''] peer-checked:after:start-[4px] peer-checked:after:translate-x-[78%]" />
          ...
          <div className="grid grid-cols-12 items-center justify-items-center gap-y-10 lg:gap-x-8">
            {pricingPlans.map((plan, idx) => (
              <RevealAnimation key={plan.id} delay={0.2 + idx * 0.1}>
                <div
                  className={cn(
                    'col-span-12 flex h-full w-full max-w-[604px] flex-col gap-6 rounded-[20px] lg:col-span-4',
                    plan.planType === 'featured'
                      ? 'relative overflow-hidden p-2.5'
                      : 'bg-white p-8 xl:px-14 xl:py-[60px] dark:bg-black',
                  )}>
                  {/* Featured plan gradient background */}
                  {plan.planType === 'featured' && (
                    <figure className="pointer-events-none absolute top-[-20%] left-[-30%] w-[810px] rotate-[30deg] select-none md:top-[-40%] md:left-[-20%] md:w-[950px] lg:top-[-20%] lg:left-[-30%] lg:w-[810px]">
                      <Image src={gradient51} alt="gradient-bg" className="h-full w-full object-cover" />
                    </figure>
                  )}
                  ...
                  <div
                    className={cn(
                      'space-y-8',
                      plan.planType === 'featured' &&
                        'relative z-10 flex h-full flex-col gap-6 rounded-[20px] bg-white p-8 xl:px-14 xl:py-[60px] dark:bg-black',
                    )}>
                    <div>
                      <h5 className="text-heading-5 mb-2 font-normal">{plan.title}</h5>
                      <div className="border-stroke-2 dark:border-stroke-6 border-b pb-8">
